...
# Hide repetitive drawFrameAxes warnings but keep errors.
try:
    cv2.setLogLevel(2)
except Exception:
    pass
...
def recording_stats(records):
    if not records:
        return None

    mm = np.asarray([
        r["translation_error_mm"]
        for r in records
    ], dtype=float)

    deg = np.asarray([
        r["rotation_error_deg"]
        for r in records
    ], dtype=float)
...
def write_recording_summary(records, path):
    stats = recording_stats(records)
...
def main(argv=None):
    ...
    #
    # SPACE / S:
    # save one Vision-vs-Tracker root-pose comparison.
    # ========================================================
    ...
    print(
        "SPACE or S = save current "
        "Vision-vs-Tracker sample"
    )
    print()
    ...
    try:
        ...
        while True:
            ...
            # =================================================
            # CURRENT ROOT POSE IN CHARUCO FIXED FRAME K
            # =================================================
            ...
            if stats is None:
                ...
                rec_text = (
                    f"REC: N={stats['count']} | "
                    f"mean "
                    f"{stats['translation_error_mm']['mean']:.1f} mm / "
                    f"{stats['rotation_error_deg']['mean']:.2f} deg"
                )
            ...
            # -----------------------------------------------
                ...
                if K_T_V is None:
                    ...
                    print(
                        "SAVE rejected: "
                        "alignment is not locked."
                    )

                elif (
                    K_T_B_tracker is None
                    or not vive_ok
                ):

                    print(
                        "SAVE rejected: "
                        "VIVE root is not valid."
                    )

                elif (
                    K_T_B_vision is None
                    or not vision_ok
                ):

                    print(
                        "SAVE rejected: "
                        "Vision root is not visible."
                    )
        ...
        if stats is not None:
            ...
            print(
                "translation [mm]: "
                f"mean={stats['translation_error_mm']['mean']:.2f}  "
                f"std={stats['translation_error_mm']['std']:.2f}  "
                f"min={stats['translation_error_mm']['min']:.2f}  "
                f"max={stats['translation_error_mm']['max']:.2f}"
            )

            print(
                "rotation [deg]:   "
                f"mean={stats['rotation_error_deg']['mean']:.3f}  "
                f"std={stats['rotation_error_deg']['std']:.3f}  "
                f"min={stats['rotation_error_deg']['min']:.3f}  "
                f"max={stats['rotation_error_deg']['max']:.3f}"
            )
